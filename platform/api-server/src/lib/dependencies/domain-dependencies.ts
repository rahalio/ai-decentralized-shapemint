...
const DOMAIN_PATH_PATTERNS: Array<{ pattern: RegExp; domain: string }> = [
  ...
  { pattern: /\/contributions\//, domain: "contributions" },
  { pattern: /\/governance\//, domain: "governance" },
  { pattern: /\/identity\//, domain: "identity" },
  { pattern: /\/incentives\//, domain: "incentives" },
  { pattern: /\/influence\//, domain: "influence" },
  { pattern: /\/models\//, domain: "models" },
  { pattern: /\/protocols\//, domain: "protocols" },
...
export async function injectDomainDependencies(
  ...
  if (!domainDeps) {
    ...
    switch (domain) {
      case "contributions":
        domainDeps = buildContributionsDomainModule(dynamoClient);
        break;
      case "governance":
      ...
      case "identity":
        ...
        break;
      case "incentives":
        domainDeps = buildIncentivesDomainModule(dynamoClient);
        break;
      case "influence":
        domainDeps = buildInfluenceDomainModule(dynamoClient);
        break;
      case "models":
        domainDeps = buildModelsDomainModule(dynamoClient);
        break;
      case "protocols":
        domainDeps = buildProtocolsDomainModule(dynamoClient);
        break;
      case "reporting":
...
export interface DomainDependencies {
  contributions: ContributionsDomainModule;
  governance: GovernanceDomainModule;
  identity: IdentityDomainModule;
  incentives: IncentivesDomainModule;
  influence: InfluenceDomainModule;
  models: ModelsDomainModule;
  protocols: ProtocolsDomainModule;
 ...
 */
export function getContributionsDeps(
  request: FastifyRequest,
): DomainDependencies["contributions"] {
  return getDomainDeps(request, "contributions");
}
...
export function getIncentivesDeps(
  request: FastifyRequest,
): DomainDependencies["incentives"] {
  return getDomainDeps(request, "incentives");
}
export function getInfluenceDeps(
  request: FastifyRequest,
): DomainDependencies["influence"] {
  return getDomainDeps(request, "influence");
}
...
export function getProtocolsDeps(
  request: FastifyRequest,
): DomainDependencies["protocols"] {
  return getDomainDeps(request, "protocols");
}
