...
describe("Postman / reporting (1:1 generated)", () => {
  ...
  it("exportInfluenceLedger", async () => {
    const url = sub("{{baseUrl}}/v1/reporting/influence-export?modelId={{modelId}}&periodStart={{periodStart}}&periodEnd={{periodEnd}}");
  ...
  it("downloadSettlementStatement", async () => {
    const url = sub("{{baseUrl}}/v1/reporting/settlement-statements/{{statementId}}/download");
    const res = await fetch(url, {
      method: "GET",
      headers: vars.accessToken ? { Authorization: `Bearer ${vars.accessToken}` } : {},
    });
